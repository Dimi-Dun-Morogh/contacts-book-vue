  ...
  <div>
      ...
      <template v-slot:body>
        <div class="inputs-wrap">
          <Input id="key" ph="key" v-model="key"
          @click.native="removeError" class="addFieldInput"
      ...
      <template v-slot:footer>
        <Button  class="modal-footer__button" @click.native="addNewField"
        :disabled="disableBtn">
...
export default {
  ...
  computed: {
    ...
    cantEditComp() {
      return this.key === 'id' ? `can't edit ${this.key}` : 'edit';
    },
    disableBtn() {
  ...
  methods: {
    ...
    removeError() {
      document.querySelectorAll('input').forEach((input) => input.parentNode.classList.remove('error'));
    },
  },
};
</script>
